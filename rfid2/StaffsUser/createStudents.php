              ...
              <div class="card mb-4">
                ...
                <div class="table-responsive p-3"  style="max-height: 400px; overflow-y: auto;">
                  <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                    ...
                    <?php
                         $query = "SELECT 
                         rs.registeredstudentid, 
                         sl.student_id_number, 
                         sl.student_name, 
                         sl.year, 
                         sl.course, 
                         rs.idpicture,
                         rc.uid  -- Include the uid from rfid_cards table
                       FROM 
                         registeredrfid_student rs
                         INNER JOIN student_list sl ON rs.student_id = sl.student_id
                         LEFT JOIN blockrfidstud bs ON rs.registeredstudentid = bs.studentregis_id
                         LEFT JOIN rfid_cards rc ON rs.rfid_card_id = rc.id  -- Join with rfid_cards on rfid_card_id
                       WHERE 
                         bs.studentregis_id IS NULL";
                            ...
                            if(mysqli_num_rows($result) > 0) {
                                while($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo "<td><a href='?action=block&Id=".$row['registeredstudentid']."' onclick='return confirm(\"Are you sure you want to block this student?\");'><i class='fas fa-fw fa-ban'></i></a></td>";
                                    echo "</tr>";
                          }
                      }
                      else
