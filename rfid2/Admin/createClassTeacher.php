                    ...
                    <tbody>
                      ...
                      $query = "
                        SELECT 
                            ...
                            rf.uid AS rfid_no,
                            inf.schoolid,
                            inf.name AS staff_name,
                            inf.department,
                            inf.category
                        FROM 
                            registaff rs
                        JOIN 
                            rfid rf ON rs.cid = rf.cid
                        JOIN 
                            information inf ON rs.did = inf.did
                        LEFT JOIN 
                            blockrfidstaff brs ON rs.stid = brs.staffregis_id
                        WHERE 
                            inf.category = 'staff' AND
                            brs.staffregis_id IS NULL";
                      $rs = $conn->query($query);
                      $num = $rs->num_rows;
                      $sn = 0;
                      ...
                      if ($num > 0) {
                        while ($rows = $rs->fetch_assoc()) {
                          ...
                          echo "
                            <tr>
                              <td>" . $sn . "</td>
                              <td><img src='../idpic/" . $rows['image'] . "' width='auto' height='50'></td>
                              <td>" . $rows['rfid_no'] . "</td>
                              <td>" . $rows['schoolid'] . "</td>
                              <td>" . $rows['staff_name'] . "</td>
                              <td>" . $rows['department'] . "</td>
                              <td>" . $rows['category'] . "</td>
                              <td><a href='?action=block&Id=" . $rows['stid'] . "'><i class='fas fa-fw fa-ban'></i></a></td>
                            </tr>";
                        }
                      } else {
